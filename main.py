...
for epoch in range(500):
    ...
    for data in data_it:
        ...
        # print(cc.shape, c_coords.shape)
        # cc_loss = relative_k * coord_criterion(cc, c_coords)*0.1
        ...
        # cc_len = cc.size()
        # cc_len = cc_len[0]*cc_len[1]
        # coord_batch_loss += cc_len*cc_loss.item()
        # print(f"{loss.item():.3}")
        y_list.append(y)
    ...
    # metrics.update({"coord loss":(coord_batch_loss/len(cc))})
    ...
    metrics = evaulate_with_affinity(valid_loader, model, criterion, affinity_criterion,coord_criterion, args.relative_k, device, pred_dis=pred_dis, use_y_mask=use_y_mask)
    ...
    # metrics = evaulate_with_affinity(test_loader, model, criterion, affinity_criterion, coord_criterion, args.relative_k,device, pred_dis=pred_dis, saveFileName=saveFileName, use_y_mask=use_y_mask)
