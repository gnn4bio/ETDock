...
class TankBindDataSet(Dataset):
    ...
    def get(self, idx):
        ...
        coords, compound_node_features, morgan, input_atom_edge_list, input_atom_edge_attr_list, pair_dis_distribution, rdkit_coord = self.compound_dict[name]
        ...
        if self.proteinMode == 0:
            data, input_node_list, keepNode = construct_data_from_graph_gvp(protein_node_xyz, protein_seq, protein_node_s, 
                                  protein_node_v, protein_edge_index, protein_edge_s, protein_edge_v,
                                  coords, compound_node_features,morgan, rdkit_coord,input_atom_edge_list, input_atom_edge_attr_list, contactCutoff=self.contactCutoff, includeDisMap=self.predDis,
...
def get_data(data_mode, logging, addNoise=None):
    pre = "/home/yiyiqiang/tankbind/TankBind-main/tankbind"
    if data_mode == "0":
