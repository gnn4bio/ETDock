...
def distance_optimize_compound_coords(coords, y_pred, protein_nodes_xyz, 
                        compound_pair_dis_constraint, total_epoch=8000, loss_function=distance_loss_function, LAS_distance_constraint_mask=None, mode=0, show_progress=False):
    # random_seed = 28
    ...
    for epoch in it:
        ...
        loss.backward()
        x.grad = x.grad + torch.randn_like(x.grad)
        optimizer.step()
