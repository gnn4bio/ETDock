...
def construct_data_from_graph_gvp(protein_node_xyz, protein_seq, protein_node_s, 
                                 ...
                                 coords, compound_node_features, morgan, rdkit_coord, input_atom_edge_list, 
    ...
    data.morgan = morgan
    data.rdkit_coord = torch.tensor(rdkit_coord, dtype=torch.float)
...
def evaulate_with_affinity(data_loader, model, criterion, affinity_criterion, coord_criterion, relative_k, device, pred_dis=False, info=None, saveFileName=None, use_y_mask=False, skip_y_metrics_evaluation=False):
    y_list = []
    y_pred_list = []
    affinity_list = []
    affinity_pred_list = []
    ...
    for data in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            # cc_loss = relative_k * coord_criterion(cc, coord)*0.1
    ...
    # metrics.update({"coord loss":(coord_batch_loss/cc_len)})
    if info is not None:
