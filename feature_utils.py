...
def extract_torchdrug_feature_from_mol(mol, has_LAS_mask=False):
    ...
    Morgan = AllChem.GetMorganFingerprintAsBitVect(mol, 2, nBits=1024, useFeatures=True)
    Morgan = torch.tensor(Morgan.ToList(), dtype=torch.float).reshape(1,-1)
    ...
    x = (coords, compound_node_features, Morgan, edge_list, edge_feature, pair_dis_distribution,coords)
    return x
